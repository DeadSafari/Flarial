...
public:
    ...
    void SettingsRender() override {
        ...
        FlarialGUI::FlarialTextWithFont(toggleX + Constraints::SpacingConstraint(0.60, textWidth), toggleY,
                                        FlarialGUI::to_wide("Right CPS").c_str(), textWidth * 6.9f, textHeight,
                                        DWRITE_TEXT_ALIGNMENT_LEADING, Constraints::SpacingConstraint(1.05, textWidth),
                                        DWRITE_FONT_WEIGHT_NORMAL);

        if (FlarialGUI::Toggle(2, toggleX, toggleY, this->settings.getSettingByName<bool>(
                "rightcps")->value)) this->settings.getSettingByName<bool>("rightcps")->value = !this->settings.getSettingByName<bool>("rightcps")->value;
